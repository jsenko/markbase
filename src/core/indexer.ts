...
import { getSchemaFieldNames } from './schema-utils.js';
...
export class Indexer {
  ...
  insertRecords(schema: Schema, records: MdRecord[]): void {
    ...
    const stmt = this.db.prepare(
      `INSERT OR REPLACE INTO "${schema.name}" (${quotedColumns}) VALUES (${placeholders})`,
    );
  ...
  reindex(schema: Schema, records: MdRecord[]): void {
    ...
    this.insertRecords(schema, records);
  }
